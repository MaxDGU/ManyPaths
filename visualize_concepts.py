...
from models import CNN, MLP, LSTM, Transformer
from constants import FEATURE_VALUES, CNN_PARAMS, MLP_PARAMS, LSTM_PARAMS, TRANSFORMER_PARAMS
...
def create_model(model_type, config):
    ...
    if model_type == "mlp":
        model = MLP(
            n_input=32*32*3 if config.get("data_type", "image") == "image" else 4, 
            n_output=1,
        ...
        model = CNN(
            n_input_channels=3 if config.get("data_type", "image") == "image" else 1,
            n_output=1,
        ...
        model = LSTM(
            n_input=16 if config.get("data_type", "image") == "bits" else 32*32*3,
            n_output=1,
            n_hidden=config["n_hidden"],
            n_layers=config["n_layers"]
        )
...
def get_activations(model, model_type, dataset, data_type="image"):
    ...
    with torch.no_grad():
        ...
        if data_type == "image":
            inputs = X_q.to(device)
            if model_type == "mlp":
                # For MLP, reshape the images
                if inputs.dim() == 4:  # [B, C, H, W]
                    inputs = inputs.reshape(inputs.size(0), -1)
            # Get activations for X_q (query set)
        ...
        activations = extract_intermediate_activations(model, model_type)
        all_activations = activations
        all_labels = y_q.numpy().flatten()
    
    return all_activations, all_labels

def extract_intermediate_activations(model, model_type):
    """Extract intermediate activations for different model types."""
    activations = {}
    
    if model_type == "mlp" or model_type == "cnn":
        # For MLP and CNN, extract activations from the sequential model
        for i, layer in enumerate(model.model):
            if isinstance(layer, torch.nn.ReLU):
                if hasattr(layer, 'output'):
                    activations[f'layer_{i}'] = layer.output.cpu().numpy()
    elif model_type == "lstm":
        # For LSTM, extract the hidden states
        if hasattr(model, 'h'):
            for i, h_i in enumerate(model.h):
                activations[f'lstm_layer_{i}'] = h_i.cpu().numpy()
    elif model_type == "transformer":
        # For Transformer, extract activations from attention layers
        if hasattr(model, 'encoder_activations'):
            for layer_name, activation in model.encoder_activations.items():
                activations[layer_name] = activation.cpu().numpy()
    
    return activations

def analyze_latent_space(model_type, configs, data_type="image"):
    """Analyze the latent space of models with different configurations."""
    # Create a dataset for testing
    dataset = MetaBitConceptsDataset(n_tasks=10, data=data_type, model=model_type)
    
    # Register hooks for each model to capture activations
    results = {}
    for config in configs:
        print(f"Analyzing {model_type} - {config['name']}...")
        model = create_model(model_type, config)
        model.to(device)
        
        # Add hooks to capture activations
        add_activation_hooks(model, model_type)
        
        # Get activations
        activations, labels = get_activations(model, model_type, dataset, data_type)
        ...
        # Remove hooks
        remove_activation_hooks(model)
    
    return results

def add_activation_hooks(model, model_type):
    """Add hooks to capture activations from different layers."""
    if model_type == "mlp" or model_type == "cnn":
        # Add hooks for MLP and CNN
        for i, layer in enumerate(model.model):
            if isinstance(layer, torch.nn.ReLU):
                layer.register_forward_hook(lambda m, i, o: setattr(m, 'output', o))
    elif model_type == "lstm":
        # For LSTM models, capture hidden states
        def lstm_hook(module, input, output):
            module.h = module.h if hasattr(module, 'h') else []
            module.h.append(output)
        
        model.register_forward_hook(lstm_hook)
    elif model_type == "transformer":
        # For Transformer models, capture attention outputs
        model.encoder_activations = {}
        
        def attention_hook(name):
            def hook(module, input, output):
                model.encoder_activations[name] = output
            return hook
        
        # Register hooks for each transformer encoder layer
        for i, layer in enumerate(model.encoder.layers):
            layer.register_forward_hook(attention_hook(f'encoder_layer_{i}'))

def remove_activation_hooks(model):
...
def create_latent_space_visualization(all_results):
    ...
    # Extract TSNE embeddings for the largest model of each type
    embeddings_by_model = {}
    for model_type in all_results.keys():
        ...
        embeddings = tsne.fit_transform(last_layer_activations)
    ...
    # Create a 2x2 grid within ax2 to show TSNE embeddings
    ...
    # Add overall title for the TSNE section
    ax2.set_title("Concept Representation in Latent Space", pad=40, 
...
def visualize_concepts():
    ...
    grid_size = (4, 4)
    cell_size = 32
    grid_image = np.ones((grid_size[0] * cell_size, grid_size[1] * cell_size, 3), dtype=np.uint8) * 255
...
def main():
    ...
    for model_type in ["mlp", "cnn", "lstm", "transformer"]:
        print(f"Analyzing {model_type} models...")
        configs = model_configs[model_type]
        results = analyze_latent_space(model_type, configs, data_type="image")
